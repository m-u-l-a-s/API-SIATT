...
export class SalaPresencialService {
  ...
  async update(id: string, updateSalaPresencialDto: CreateSalaPresencialDto) {
    const salaUpdate = await this.salaPresencialRepository.findOneBy({
      id: id,
    });
    if (salaUpdate != null) {
      salaUpdate.identificacao = updateSalaPresencialDto.identificacao;
      salaUpdate.endereco = updateSalaPresencialDto.endereco;
  ...
  async remove(id: string) {
    const salaPresencial: SalaPresencialEntity =
      await this.salaPresencialRepository.findOneBy({ id: id });

    salaPresencial;

    const reunioes: ReuniaoEntity[] = await this.reuniaoRepository.findBy({
      salaPresencial: salaPresencial,
    });

...
export class ReuniaoService {
  ...
  async criarReuniaoEntity(
    reuniaoDTO: CreateReuniaoDto,
  ): Promise<ReuniaoEntity> {
    ...
    try {
      reuniao.solicitante = await this.usuarioService.findOneByEmail(
        reuniaoDTO.solicitanteEmail,
      );
    } catch (error) {
  ...
  async criarReuniaoFisica(
    reuniao: ReuniaoEntity,
    reuniaoDTO: CreateReuniaoDto,
  ) {
    const salaPresencial: SalaPresencialEntity =
      await this.salaPresencialService.findOne(reuniaoDTO.presencial);
    if (salaPresencial.ocupacaoMax < reuniaoDTO.participantes.length) {
      throw new Error(
        'Número de participantes excede a ocupação máxima da sala',
      );
    }
  ...
  async createAta(id: string, reuniao: CreateReuniaoDto) {
    ...
    if (reuniao.joinUrl) {
      ...
      local = (await this.salaPresencialService.findOne(reuniao.presencial))
        .local;
    }
    ...
    const doc = await patchDocument(fs.readFileSync(modeloAta), {
      patches: {
        RPC_PROGRAMA_AREA: {
          ...
          children: [new Paragraph({ text: reuniao.titulo })],
        },
        RPC_ASSUNTO: {
          type: PatchType.DOCUMENT,
          children: [new Paragraph({ text: reuniao.pauta })],
        },
        ...
        RPC_LOCAL: {
          type: PatchType.DOCUMENT,
          children: [new Paragraph({ text: local })],
        },

        RPC_RELATOR: {
          type: PatchType.DOCUMENT,
          children: [new Paragraph({ text: reuniao.solicitanteEmail })],
        },
    ...
    const reuniaoData: ReuniaoEntity = await this.reuniaoRepository.findOneBy({
      id: id,
    });
  ...
  async update(id: string, reuniaoDTO: CreateReuniaoDto, token?: string) {
    const reuniao: ReuniaoEntity = await this.reuniaoRepository.findOneBy({
      id: id,
    });
    if (reuniao) {
      if (
        reuniao.categoria == Categoria.HIBRIDA ||
        reuniao.categoria == Categoria.VIRTUAL
      ) {
        if (!token) {
          throw new UnauthorizedException('Token inválido ou vazio!');
        }
        reuniaoDTO.dataHora = new Date(reuniaoDTO.dataHora);
        const respZoom = await this.zoomServico.updateMeeting(
          reuniaoDTO,
          token,
        );
      ...
      try {
        ...
        if (reuniaoDTO.presencial !== '') {
          try {
            reuniao.salaPresencial = await this.salaPresencialService.findOne(
              reuniaoDTO.presencial,
            );
          } catch (error) {
            console.log(error);
          }
  ...
  async remove(id: string, token?: string): Promise<HttpStatusCode> {
    ...
    if (
      ...
      const resp = await this.zoomServico.deleteMeeting(
        reuniao.zoomMeetingId,
        token,
      );
      if (resp.status == 401) {
